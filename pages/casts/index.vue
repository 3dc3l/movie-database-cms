        ...
        <table class="table">
			...
			<tbody v-if="res.length > 0">
				<tr v-for="(data, key) in res" :key="key">
                   ...
                   <td>{{ (data.middle_name) ? data.middle_name : 'N/A' }}</td>
                   <td>{{ data.last_name }}</td>
                   <td>{{ (data.suffix) ? data.suffix : 'N/A' }}</td>
                   <td>{{ data.stage_name }}</td>
                   <td class="buttons">
						<div class="wrapper">
							<nuxt-link :to="`/casts/${data.id}/update`" class="item edit">
    ...
    export default {
        ...
        methods: {
            ...
            fetchData (status = null) {
                const me = this
                me.$axios.get('api/casts').then(res => {
                    me.res = res.data
                }).catch(err => {
